...
README.md: docs/index.rst simple_perms/__init__.py
	@set -euxo pipefail
	make -C docs
...
nix-checks:  ## Checks that depend on a *NIX OS/FS
	...
	if [ "$$OSTYPE" == "msys" ]; then  # e.g. Git bash on Windows
		...
		if [[ "$$FSTYPE" =~ ^(vfat|vboxsf)$$ ]]; then
			...
			UNRELIABLE_PERMS=""
			set -x
			simple-perms $(perm_checks)  # if this errors, run `simple-perms -m ...` for auto fix
			test -z "$$( find . \( -type d -name '.venv*' -prune \) -o \( -iname '*.sh' ! -executable -print \) )"
		fi
	fi
	test -z "$$UNRELIABLE_PERMS"  # the tests will only work with reliable permission bits
